...
(the `rcarmo/ghostty-web` fork, installed from GitHub by pinned tag — the npm
package of the same name is the unmaintained upstream) with the served
`/ghostty-vt.wasm` asset, `terminalConnection.ts` owns the
