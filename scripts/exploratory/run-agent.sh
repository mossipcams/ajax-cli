...
write_agent_status() {
  ...
  EXIT_CODE="$exit_code" ERROR_MESSAGE="$error_message" MODEL="$MODEL" BUDGET_MINUTES="$BUDGET_MINUTES" FINALIZATION_RESERVE_MINUTES="$FINALIZATION_RESERVE_MINUTES" \
...
set +e
timeout --signal=TERM --kill-after=60s "${BUDGET_SECONDS}s" \
  "$AGENT_BIN" \
  --print \
  --trust \
  --approve-mcps \
