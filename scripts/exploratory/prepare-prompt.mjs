...
function main() {
  ...
  const explorationMinutes = Math.max(1, budgetMinutes - finalizationReserveMinutes);
...
- Base URL: ${BASE_URL}
- Time budget: **${budgetMinutes} minutes maximum**, not a target. Reserve the **final ${finalizationReserveMinutes} minutes exclusively** for artifact finalization (findings, observations, memory-delta, run.json). Stop active exploration after ~${explorationMinutes} minutes or when stopping criteria apply.
