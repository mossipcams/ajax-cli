...
impl TaskSessionState {
    ...
    pub(super) fn append_to_log(
        ...
        match web_session_store::append_events(&self.state_dir, &self.qualified_handle, &filtered) {
            Ok(()) => {
            ...
            Err(error) => {
                ...
                self.evidence.transcript_durability_fault = Some(message.clone());
                self.evidence.pending_transcript_error_snapshot = true;
