...
| First dial seeds history via `capture-pane -p -e -S -10000 -E -1` (no `-J`); after client resizes, bridge waits 150ms quiet (within 500ms overall); TaskTerminal enables `scrollOnEraseInDisplay` only for the seeded-open window so attach `CSI 2 J` pushes seeded viewport into scrollback (no CRLF pad), then disables it after seed reveal so live agent/tmux clears do not dump viewport frames into scrollback; unseeded auto-reconnect (`?seed=0`) disables it immediately; seeded manual reconnect re-enables it before the new seed | Product | `terminal_pty.rs:199-210,336-342,505-521`; `TaskTerminal.tsx` (`scrollOnEraseInDisplay` seed-window latch); `terminalConnection.ts:154-180,198-202`; tests: `seed_history_query_parsing`, `isolated_attach_plan_seeds_browser_scrollback_from_task_window`, `resize_settle_deadline_returns_remaining_until_quiet_elapsed`, `history_capture_preserves_display_wrapping`, `captured_history_frame_bytes_does_not_append_pad_crlfs`, `TaskTerminal.test.tsx` (`scrollOnEraseInDisplay`), `scrollbackOverwriteProbe.test.ts`, `terminalConnection.test.ts:141,146,175,207` |
