// Diff Review mobile-webkit smoke: task detail left swipe opens the projection
...
async function touchDragLeft(target: Locator, dx: number) {
  await target.evaluate(async (el, distance) => {
...
test("task detail swipe-left opens Diff Review chrome", async ({ page }) => {
  ...
  await touchDragLeft(page.getByTestId("mobile-chrome-header"), 120);
