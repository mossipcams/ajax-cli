      ...
      {entries.map((entry) => {
        if (entry.kind === "prose" && entry.role === "user") {
          return (
            <article
              key={entry.id}
              className="session-said"
              data-testid="session-message-user"
            >
              {entry.text}
            </article>
          );
        ...
        if (entry.kind === "tools") {
          ...
          return (
            <div key={entry.id} className="session-tools" data-testid="session-tools">
              {settled.map((call) => (
                <div
                  key={call.callId}
        ...
        if (entry.kind === "plan") {
          return (
            <section key={entry.id} className="session-plan" data-testid="session-plan">
