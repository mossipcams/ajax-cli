...
export type SessionComposerSpeechDeps = {
  handle: string;
  /** Append finalized speech text into the composer draft. */
  appendDraftText: (text: string) => void;
  /** Undo last speech inserts by character count (DEL semantics). */
  deleteDraftBackward: (charCount: number) => void;
};

export function useSessionComposerSpeech(deps: SessionComposerSpeechDeps): {
  ...
  const { handle, appendDraftText, deleteDraftBackward } = deps;
  ...
  const appendRef = useRef(appendDraftText);
  appendRef.current = appendDraftText;
  const deleteRef = useRef(deleteDraftBackward);
  deleteRef.current = deleteDraftBackward;
  ...
  const undoInsertedSpeech = () => {
    ...
    if (charCount > 0) deleteRef.current(charCount);
  };
  ...
  const finalizeMic = () => {
    ...
    if (current.state !== "listening" && current.state !== "pause_pending") {
      return;
    }
      ...
      handle,
      {
        ...
        onFinal: (sequence, text) => {
          ...
          if (isStandaloneStartOver(text)) {
            ...
            if (canHandleControl) undoInsertedSpeech();
            return;
          }
          ...
          if (
            ...
            if (delta) {
              appendRef.current(delta);
              insertedSpeechRef.current.push({ text: delta, bracketed: false });
            }
          }
        },
