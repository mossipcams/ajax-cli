//! Self-signed TLS identity for the mobile web companion.
//!
//! Service workers and Web Push require a secure context, so the companion
//! serves HTTPS. The identity is generated once and persisted next to the Ajax
...
pub(crate) fn load_or_create_identity(dir: &Path) -> Result<TlsIdentity, CliError> {
    ...
    std::fs::create_dir_all(dir)
        .map_err(|error| CliError::CommandFailed(format!("web tls dir create failed: {error}")))?;
    write_private(&key_path, &identity.key_pem)?;
    std::fs::write(&cert_path, &identity.cert_pem)
        .map_err(|error| CliError::CommandFailed(format!("web tls cert write failed: {error}")))?;
    Ok(identity)
}

/// Generates a fresh self-signed identity valid for `localhost` and, when it
/// can be detected, the machine's primary LAN address.
fn generate_identity() -> Result<TlsIdentity, CliError> {
    ...
    let certified = rcgen::generate_simple_self_signed(subject_alt_names).map_err(|error| {
        CliError::CommandFailed(format!("web tls cert generation failed: {error}"))
    })?;
    Ok(TlsIdentity {
...
/// Detects the machine's primary outbound IPv4 address without sending any
/// packets: a connected UDP socket simply records the route's source address.
...
pub(crate) fn tls_server_config(
    ...
    let certs = CertificateDer::pem_slice_iter(identity.cert_pem.as_bytes())
        ...
        .map_err(|error| CliError::CommandFailed(format!("web tls cert parse failed: {error}")))?;
    if certs.is_empty() {
    ...
    let key = PrivateKeyDer::from_pem_slice(identity.key_pem.as_bytes())
        .map_err(|error| CliError::CommandFailed(format!("web tls key parse failed: {error}")))?;

    let provider = Arc::new(rustls::crypto::ring::default_provider());
    let config = rustls::ServerConfig::builder_with_provider(provider)
        .with_safe_default_protocol_versions()
        .map_err(|error| {
            CliError::CommandFailed(format!("web tls protocol setup failed: {error}"))
        })?
        .with_no_client_auth()
        .with_single_cert(certs, key)
        .map_err(|error| CliError::CommandFailed(format!("web tls config failed: {error}")))?;
    Ok(Arc::new(config))
}
...
pub(crate) fn write_private(path: &Path, contents: &str) -> Result<(), CliError> {
    std::fs::write(path, contents)
        .map_err(|error| CliError::CommandFailed(format!("web tls key write failed: {error}")))?;
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
        std::fs::set_permissions(path, std::fs::Permissions::from_mode(0o600)).map_err(
            |error| CliError::CommandFailed(format!("web tls key chmod failed: {error}")),
        )?;
    }
    Ok(())
}
