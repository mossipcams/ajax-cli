//! Web Push support for the mobile companion: VAPID identity, browser push
//! subscriptions, and encrypted notification delivery.
...
pub(crate) fn load_subscriptions(dir: &Path) -> Vec<PushSubscription> {
...
/// Persists the full set of push subscriptions to `dir`.
fn save_subscriptions(dir: &Path, subscriptions: &[PushSubscription]) -> Result<(), CliError> {
    std::fs::create_dir_all(dir).map_err(|error| {
        CliError::CommandFailed(format!("web push subscription dir create failed: {error}"))
    })?;
    ...
    std::fs::write(dir.join(SUBSCRIPTIONS_FILE), json).map_err(|error| {
        CliError::CommandFailed(format!("web push subscription write failed: {error}"))
    })
}
...
pub(crate) fn add_subscription(dir: &Path, subscription: PushSubscription) -> Result<(), CliError> {
...
pub(crate) fn remove_subscription(dir: &Path, endpoint: &str) -> Result<(), CliError> {
    let mut subscriptions = load_subscriptions(dir);
...
pub(crate) fn load_or_create_vapid_keys(dir: &Path) -> Result<VapidKeys, CliError> {
    ...
    let private_pem = match std::fs::read_to_string(&path) {
        ...
        _ => {
            ...
            std::fs::create_dir_all(dir).map_err(|error| {
                CliError::CommandFailed(format!("web push key dir create failed: {error}"))
            })?;
...
/// Generates a fresh P-256 (prime256v1) private key in PKCS#8 PEM form.
fn generate_vapid_pem() -> Result<String, CliError> {
    let key = rcgen::KeyPair::generate_for(&rcgen::PKCS_ECDSA_P256_SHA256).map_err(|error| {
        CliError::CommandFailed(format!("web push key generation failed: {error}"))
    })?;
    Ok(key.serialize_pem())
}

/// Derives the uncompressed public key bytes from a VAPID PEM private key.
fn derive_public_key(private_pem: &str) -> Result<Vec<u8>, CliError> {
    let builder = web_push::VapidSignatureBuilder::from_pem_no_sub(private_pem.as_bytes())
        .map_err(|error| {
            CliError::CommandFailed(format!("web push key is not a valid VAPID key: {error}"))
        })?;
...
/// Serializes a notification into the JSON payload the service worker expects.
...
/// True when the push service reports a subscription as permanently gone.
...
/// Builds a VAPID-signed, encrypted Web Push message for one subscription.
fn build_push_message(
    vapid_pem: &str,
    subscription: &PushSubscription,
    payload: &[u8],
) -> Result<web_push::WebPushMessage, CliError> {
    ...
    let mut signature_builder =
        web_push::VapidSignatureBuilder::from_pem(vapid_pem.as_bytes(), &subscription_info)
            .map_err(|error| {
                CliError::CommandFailed(format!("web push signature setup failed: {error}"))
            })?;
    signature_builder.add_claim("sub", "mailto:ajax-cockpit@localhost");
    let signature = signature_builder.build().map_err(|error| {
        CliError::CommandFailed(format!("web push signature build failed: {error}"))
    })?;
    ...
    message_builder
        .build()
        .map_err(|error| CliError::CommandFailed(format!("web push message build failed: {error}")))
}
...
pub(crate) fn send_to_all(dir: &Path, notification: &PushNotification) -> Result<usize, CliError> {
    ...
    let runtime = tokio::runtime::Builder::new_current_thread()
        .enable_all()
        .build()
        .map_err(|error| CliError::CommandFailed(format!("web push runtime failed: {error}")))?;
...
mod tests {
    ...
    fn subscriptions_are_stored_deduplicated_and_removed() {
        ...
        // Re-subscribing the same endpoint must not create a duplicate.
        add_subscription(&dir, subscription("https://push.example/a")).unwrap();
    ...
    fn transient_errors_are_not_classified_as_gone() {
        // `EndpointNotValid` / `EndpointNotFound` carry an `ErrorInfo` the
        // crate does not export, so they cannot be constructed here; the
        // gone-classification arms are verified by the compiler. Transient
        // failures must never trigger subscription pruning.
    ...
    fn build_push_message_rejects_invalid_subscription_keys() {
        ...
        // A syntactically present but cryptographically invalid p256dh must
