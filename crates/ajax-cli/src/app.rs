...
pub use crate::context::CliContextPaths;
use crate::{agent_runtime, cli, context, execution_dispatch, snapshot_dispatch};
#[cfg(test)]
#[cfg(feature = "interactive")]
pub(crate) use crate::{
    execute_pending_cockpit_action, execute_pending_cockpit_action_with_task_session,
    handle_pending_cockpit_result, refresh_cockpit_snapshot, render_cockpit_command,
    tui_cockpit_action, tui_cockpit_confirmed_action,
};
#[cfg(test)]
pub(crate) use crate::{parent_directory_available, render_drop_command, render_task_command};
#[cfg(test)]
#[cfg(feature = "interactive")]
mod cockpit_actions {
    pub(crate) use crate::cockpit_actions::*;
}
#[cfg(feature = "interactive")]
use crate::{bgtmux, cockpit_backend, task_session};
use ajax_core::{
...
impl CliError {
    ...
    pub(crate) fn after_state_change(self) -> Self {
        match self {
            CliError::CommandFailed(message) => CliError::CommandFailedAfterStateChange(message),
            error => error,
        }
