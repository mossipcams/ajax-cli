...
fn live_new_execute_records_task_and_persists_it_to_sqlite_state() {
    ...
    assert!(lifecycle_log.contains(&format!(
        "args=-C {repo} worktree add -b ajax/fix-login {worktree} origin/main",
    ...
    assert!(lifecycle_log.contains(&format!(
        "args=send-keys -t ajax-web-fix-login:task (if [ -f package.json ] && [ -f .husky/pre-commit ]; then npm exec --yes husky; fi) >/dev/null 2>&1; ajax-cli __agent-runtime --task-id web/fix-login --state-root {state_root} -- codex --cd {worktree} Enter",
