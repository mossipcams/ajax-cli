...
fn tmux_task_parser_detects_wrong_path() {
    let task = TmuxAdapter::parse_task_window_status("task", "/tmp/worktree", "task\t/tmp/wrong\n");
...
fn cursor_model_intents_match_thinking_catalog_and_bracket_issue_1013() {
    ...
    let bracket =
        parse_cursor_model_intent("claude-opus-5[thinking=true,effort=medium,fast=false]").unwrap();
...
fn cursor_unspecified_spawn_satisfied_accepts_spawn_default_and_rejects_fast() {
    use crate::adapters::agent::{cursor_unspecified_spawn_satisfied, CURSOR_DEFAULT_SPAWN_MODEL};
