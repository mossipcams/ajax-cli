...
pub fn plan<R: Registry>(context: &CommandContext<R>) -> CommandPlan {
...
pub fn execute<R: Registry>(
    ...
    for candidate in &candidates {
        ...
        .map_err(|error| (error, state_changed))?;
        match crate::slices::drop::complete(context, candidate, &observation)
            .map_err(|error| (error, state_changed))?
        {
            crate::slices::drop::DropTaskCompletion::Removed
            | crate::slices::drop::DropTaskCompletion::TeardownIncomplete { .. } => {
                state_changed = true;
            }
        }
    }
