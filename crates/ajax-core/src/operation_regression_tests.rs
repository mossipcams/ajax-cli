...
mod tests {
    ...
    use crate::{
        ...
        slices::drop::{
            complete as complete_drop_task_operation, execute as execute_drop_task_operation,
            plan_operation as plan_drop_task_operation, DropTaskCompletion,
        },
    };

    #[derive(Clone, Copy, Debug, Eq, PartialEq)]
    enum RegressionCommandKind {
        Resume,
        Review,
        Repair,
        Ship,
    ...
    fn plan_start_task_operation<R: Registry>(
        context: &CommandContext<R>,
        request: NewTaskRequest,
    ) -> Result<(crate::models::TaskIntent, CommandPlan), CommandError> {
        crate::slices::start::plan(context, request)
    }

    fn execute_start_task_operation<R: Registry>(
        ...
        crate::slices::start::execute(context, runner, request, plan, confirmed, open_mode)
    }

    fn plan_task_command_operation<R: Registry>(
        context: &CommandContext<R>,
        kind: RegressionCommandKind,
        qualified_handle: &str,
        open_mode: OpenMode,
    ) -> Result<CommandPlan, CommandError> {
        match kind {
            RegressionCommandKind::Resume => {
                crate::slices::resume::plan(context, qualified_handle, open_mode)
            }
            RegressionCommandKind::Review => crate::slices::review::plan(context, qualified_handle),
            RegressionCommandKind::Repair => {
                crate::slices::repair::plan(context, qualified_handle, open_mode)
            }
            RegressionCommandKind::Ship => crate::slices::ship::plan(context, qualified_handle),
        }
    }

    fn execute_task_command_operation<R: Registry>(
        context: &mut CommandContext<R>,
        kind: RegressionCommandKind,
        qualified_handle: &str,
        plan: &CommandPlan,
        ...
        match kind {
            RegressionCommandKind::Resume => {
                crate::slices::resume::execute(context, qualified_handle, plan, confirmed, runner)
            }
            RegressionCommandKind::Review => {
                crate::slices::review::execute(context, qualified_handle, plan, confirmed, runner)
            }
            RegressionCommandKind::Repair => {
                crate::slices::repair::execute(context, qualified_handle, plan, confirmed, runner)
            }
            RegressionCommandKind::Ship => {
                crate::slices::ship::execute(context, qualified_handle, plan, confirmed, runner)
            }
        }
    }

    fn execute_sweep_cleanup_operation<R: Registry>(
        context: &mut CommandContext<R>,
        confirmed: bool,
        runner: &mut impl CommandRunner,
    ) -> Result<(Vec<CommandOutput>, bool), (CommandError, bool)> {
        crate::slices::tidy::execute(context, confirmed, runner)
    }
    ...
    fn context_with_cleanable_task() -> CommandContext<InMemoryRegistry> {
        ...
        crate::lifecycle::mark_reviewable(&mut task).unwrap();
        crate::lifecycle::mark_merged(&mut task).unwrap();
        crate::lifecycle::mark_cleanable(&mut task).unwrap();
        ...
        context
            .registry
            .update_lifecycle(&task_id, LifecycleStatus::Cleanable)
            .unwrap();
        context
    }
    ...
    fn context_with_reviewable_task() -> CommandContext<InMemoryRegistry> {
        ...
        context
            .registry
            .update_lifecycle(&task_id, LifecycleStatus::Reviewable)
            .unwrap();
        context
    }
    ...
    fn operation_kernel_refuses_blocked_plan_without_running_commands() {
        ...
        assert_eq!(
            external_plan::execute(&blocked_plan, true, &mut runner),
    ...
    fn operation_kernel_requires_confirmation_before_running_risky_plan() {
        ...
        assert_eq!(
            external_plan::execute(&confirmation_plan, false, &mut runner),
    ...
    fn operation_kernel_surfaces_nonzero_exit_after_running_the_failing_command() {
        ...
        assert_eq!(
            external_plan::execute(&failing_plan, true, &mut runner),
    ...
    fn operation_kernel_returns_outputs_for_successful_plan() {
        ...
        assert_eq!(
            external_plan::execute(&success_plan, true, &mut runner).unwrap(),
            vec![
                CommandOutput {
                    status_code: 0,
                    stdout: "ok".to_string(),
    ...
    fn task_command_operation_plans_use_operator_titles() {
        ...
        let cases = [
            (RegressionCommandKind::Resume, "open task: web/fix-login"),
            (RegressionCommandKind::Review, "diff task: web/fix-login"),
            (RegressionCommandKind::Repair, "repair task: web/fix-login"),
            (RegressionCommandKind::Ship, "merge task: web/fix-login"),
        ];
    ...
    #[test]
    fn resume_rejects_substrate_loss_after_available_decision() {
        let mut context = context_with_reviewable_task();
        let resume_plan = plan_task_command_operation(
            &context,
            RegressionCommandKind::Resume,
        ...
        let mut runner = RecordingQueuedRunner::new(Vec::new());
    ...
    fn claude_waiting_context() -> CommandContext<InMemoryRegistry> {
        ...
        let task_id = TaskId::new("web/fix-login");
        {
            ...
            crate::lifecycle::hydrate_lifecycle_status(task, LifecycleStatus::Active);
        }
        let task = context.registry.get_task_mut(&task_id).unwrap();
    ...
    fn repair_operation_records_tests_failed_on_check_failure() {
        ...
        context
            .registry
            .get_task_mut(&TaskId::new("web/fix-login"))
            .map(|task| crate::lifecycle::hydrate_lifecycle_status(task, LifecycleStatus::Active))
            .unwrap();
        let repair_plan = plan_task_command_operation(
            &context,
            RegressionCommandKind::Repair,
    ...
    fn drop_operation_removes_failed_or_orphaned_tasks_when_resources_are_absent() {
        for lifecycle_status in [LifecycleStatus::Error, LifecycleStatus::Orphaned] {
            ...
            crate::lifecycle::hydrate_lifecycle_status(&mut task, lifecycle_status);
            context.registry.create_task(task).unwrap();
    ...
    fn drop_retry_repeats_receipted_step_when_fresh_observation_finds_resource_present() {
        ...
        context
            ...
            .map(|task| {
                crate::lifecycle::hydrate_lifecycle_status(
                    task,
