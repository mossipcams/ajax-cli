...
- A second verifier **agent** (host deterministic verifier is in scope).
- Base/head replay.
...
- [x] Reduce the workflow to one nightly scout process and tighten its
...
- [x] Redesign: primary→fallback activation on seed/preflight failure, hard
      finalization reserve on scout timeout, verifier confirmation gate
      (scout `confirmed` → observation unless host `deterministic-verifier`
      evidence exists), classify/file only independently verified findings.
...
- 2026-08-28: redesign contract validation on branch `ajax/nightly-exploratory-redesign`
  — `node --test scripts/exploratory-helpers.test.mjs scripts/exploratory-missions.test.mjs`
  pass (fallback seed/preflight, verifier gate, finalization reserve metadata,
  cli.json verifier write deny). Fixture pipeline still passes with simulated
  host verifier evidence (`validate-run.mjs --fixture`).

## Live verifier boundary

The redesign separates **scout** (Cursor Agent) from **verifier** (host):

- Scout may write scout evidence and mark hypotheses `confirmed` in `findings.json`.
- `.github/exploratory/cli.json` denies scout writes to
  `exploratory-results/verifier/**`.
- `validate-run.mjs` demotes scout-only `confirmed` findings to `observation`
  unless `verifier.json` carries a matching `deterministic-verifier` entry with
  `reproductionSuccesses >= 2` and on-disk files under `exploratory-results/verifier/`.
- `classify-findings.mjs` and `file-issues.mjs` operate only on findings that
  survive that gate.

**Current gap:** the confirmation gate and eligibility checks are live in the
branch scripts, but the scheduled workflow does not yet include a host step that
generates `verifier.json` and verifier artifacts after the scout finishes. Until
that producer lands, nightly runs on the redesigned workflow will classify and
file **zero** scout-reported defects (demoted to observations) rather than
auto-filing unverified agent confirmations like run 33099612279 did under the old
workflow.
