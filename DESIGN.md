...
- **Attention chips:** Soft Steel Blue fill when active, count in the same pill; a chip exists only for a band the host populated. Repo scoping is a native `<select>` sharing the chip wrap row — not a pill per repo.
