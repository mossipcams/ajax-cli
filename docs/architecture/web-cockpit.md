...
(`ChatSurface.tsx`). Touches started in overflowing `.session-tool-output` and
`.session-diff-body` blocks belong to those inner scrollers and never arm the
workspace page swipe; markdown code blocks retain native horizontal scrolling.
